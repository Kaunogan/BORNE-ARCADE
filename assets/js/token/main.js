...
readLetter(function() {
  if (letter != alphabet[d - 1].toString()) {
    console.log('je clear');
    clearToken();
    writeLetter(alphabet[d - 1].toString())
  }
});
  ...
  readToken(function() {
    ...
    if (!same) {
      if (alphabet[letter] == undefined) {
        ...
        if (decMsg == alphabet[d - 1].toString()) {
...
function writeLetter(letter) {
  fs.writeFile("conf/letter.conf", letter, function(err) {
    if (err) {
      return console.log(err);
    }
  });
...
function clearToken() {
  fs.writeFile("conf/tokenUsed.conf", "", function(err) {
    if (err) {
      return console.log(err);
    }
  });
...
function readLetter(callback) {
  fs.readFile("conf/letter.conf", "utf-8", (err, data) => {
    if (err) {
      console.error(err);
      return;
    }
