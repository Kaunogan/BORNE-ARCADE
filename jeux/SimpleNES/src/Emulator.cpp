...
namespace sn
{
    ...
    void Emulator::run(std::string rom_path)
    {
        ...
        m_window.create(sf::VideoMode(sf::VideoMode::getDesktopMode().width,sf::VideoMode::getDesktopMode().height),"SimpleNES");
        m_window.setVerticalSyncEnabled(true);
        m_emulatorScreen.create(NESVideoWidth, NESVideoHeight, m_screenScale, sf::Color::Black);
        ...
        while (m_window.isOpen())
        {
            while (m_window.pollEvent(event))
            {
                ...
                {
                    for (int i = 0; i < 39781; ++i) //Around one frame
                    {
                        //PPU
                        m_ppu.step();
                        m_ppu.step();
