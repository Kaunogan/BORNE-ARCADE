...
int main()
{
  ...
  RenderWindow app(VideoMode(W, H), "Asteroids!"/*, sf::Style::Fullscreen*/);
  ...
  background.setScale((float)app.getSize().x/(float)t2.getSize().x, (float)app.getSize().y/(float)t2.getSize().y);
  ...
  while (app.isOpen())
  {
    ...
    //////draw lives//////
    _full_text.setString("LIVES : " + std::to_string(lives));
    sf::FloatRect box = _full_text.getLocalBounds();
    _full_text.setOrigin(box.left + box.width / 2.0f, box.top + box.height / 2.0f);
    _full_text.setPosition(W / 1.07, H / 17);
    _full_text.setFillColor(sf::Color::Green);
    app.draw(_full_text);
